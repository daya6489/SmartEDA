...
#'
#'
#' @description This function automatically scans through each variable and creates normal QQ plot also adds a line to a normal quantile-quantile plot.
#' @usage ExpOutQQ(data,nlim=NULL,fname=NULL,Page=NULL,sample=NULL)
...
ExpOutQQ = function(data,nlim=NULL,fname=NULL,Page=NULL,sample=NULL) {
  ...
  if (!is.null(fname)) {
    ...
    {
      pdf(file=filname,width=10,height = 6,onefile=T,title="Summary analysis",paper = "a4r")
      print(plot_l)
      dev.off()
    }
  }
